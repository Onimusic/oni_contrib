def font_types_list(for_choices=False) -> tuple:
    ...
    Args:
        for_choices: Limitação de opções desejadas. Opcional.

    Returns: A full tuple of font objects or a tuple of ((key1, name1),(key2, name2).....(keyN, nameN))
    """
    fonts = (
        {'name': 'Open Sans',
    ...
    return ((str(key), item['name']) for key, item in enumerate(fonts)) if for_choices else fonts
...
def _get_font_details(font_id: int = 0, item: str = 'url') -> str:
    ...
    Args:
        font_id: Identificador da fonte
        item: item inside details to be returned

    Returns: url da fonte desejada
